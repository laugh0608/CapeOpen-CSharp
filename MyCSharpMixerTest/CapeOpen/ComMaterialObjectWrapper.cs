...
internal class ComMaterialObjectWrapper : CapeObjectBase,
    ...
    ICapeThermoCompoundsCOM, ICapeThermoPhasesCOM,
    ICapeThermoUniversalConstantCOM, ICapeThermoEquilibriumRoutineCOM,
    ICapeThermoPropertyRoutineCOM
{
    ...
    /// </remarks>
    /// <value>
    /// Te names of the compounds in the material object.
    /// </value>
    /// <exception cref ="ECapeUnknown">The error to be raised when other error(s),  specified for this operation, are not suitable.</exception>
    ...
    /// <exception cref = "ECapeUnknown">The error to be raised when 
    /// other error(s), specified for this operation, are not suitable.</exception>
    void ICapeThermoMaterialCOM.GetOverallTPFraction(ref  double temperature, 
        ref  double pressure, ref  object composition)
    {
        double[] temp = null;
    ...
    void ICapeThermoPropertyRoutineCOM.CalcAndGetLnPhi(string phaseLabel, double temperature, double pressure, object moleNumbers, int fFlags, ref  object lnPhi, ref  object lnPhiDT, ref  object lnPhiDP, ref  object lnPhiDn)
    {
        ...
        if (fFlags % 2 == 1)
        {
            ...
            if (fFlags / 8 == 1)
            {
                flags |= CapeFugacityFlag.CAPE_MOLE_NUMBERS_DERIVATIVES;
                fFlags -= 8;
            }
            if (fFlags / 4 == 1) 
            {
                flags |= CapeFugacityFlag.CAPE_P_DERIVATIVE;
                fFlags -= 4;
            }
            if (fFlags / 2 == 1) flags |= CapeFugacityFlag.CAPE_T_DERIVATIVE;
        }
